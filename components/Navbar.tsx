...
export const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate }) => {
  ...
  const inactiveLink = `${baseLink} border-transparent text-slate-700 hover:border-indigo-300 hover:text-indigo-700`;
  const activeLink = `${baseLink} border-indigo-500 text-indigo-700`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            <button type="button" onClick={() => onNavigate('profile')} className="text-sm font-medium text-slate-600 hover:text-indigo-700 transition-colors">My Profile</button>
            <button type="button" onClick={() => onNavigate('consultancy')} className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg text-white bg-gradient-to-r from-indigo-600 to-blue-500 hover:opacity-95 transition-opacity">Open Consultancy</button>
          </div>
        </div>
      </div>
    </nav>
