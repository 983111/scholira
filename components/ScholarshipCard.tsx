...
export const ScholarshipCard: React.FC<{ scholarship: Scholarship }> = ({ scholarship }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col h-full group">
      <div className="p-6 flex-1">
        <div className="flex justify-between items-start mb-4">
          <div className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100">{scholarship.provider}</div>
          <span className="text-xs text-slate-500 font-medium">{scholarship.location}</span>
        </div>
        <h3 className="text-lg font-bold text-slate-900 mb-3 group-hover:text-indigo-700 transition-colors line-clamp-2">{scholarship.name}</h3>
      ...
      <div className="px-6 py-4 bg-indigo-50/50 border-t border-indigo-100 rounded-b-2xl flex justify-between items-center">
        <span className="text-xs text-slate-500 font-medium">Verified source</span>
        <a href={targetUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-indigo-700 hover:text-indigo-900">Apply / View</a>
      </div>
    </div>
  );
};
